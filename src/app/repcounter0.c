...
void repcounter_execute_all_pending () {
    cmd_t cmd_type;
    uint cmd1;
    while (sharedlog_getnext(&cmd_type, &cmd1) == STATUS_OK) {
        counter_perform_cmd(cmd_type, cmd1);
    }
}
...
void repcounter_init (uint client_id) {
    uint addr;
    counter_value_set(0);
    sharedlog_init(client_id);
}

int repcounter_read () {
    // After appending a 'read' command, the log local copy will be up-to-date.
    if (sharedlog_append(CMD_REPCOUNTER_READ,0) == STATUS_OK) {
...
int repcounter_inc (uint x) {
    if (sharedlog_append(CMD_REPCOUNTER_INC,x) == STATUS_OK) {
        repcounter_execute_all_pending();
        return STATUS_OK;
    } else {
